...
import time
import serial
import serial.tools.list_ports  # Add this import
import threading
import queue
...
# Print available serial ports before asking about disabling print
...
def select_serial_port():
    ...
    while True:
        try:
            idx = int(input("Select COM port by number: "))
            if 0 <= idx < len(ports):
...
def ask_disable_print():
    resp = input("Disable screen printing? (y/N): ").strip().lower()
    if resp == "y":
        import builtins
...
# Open serial connection (moved after port selection)
arduino = None  # Placeholder, will be set in main
...
class XInputJoystick(event.EventDispatcher):
    ...
    def dispatch_axis_events(self, state):
        ...
        for axis, type in list(axis_fields.items()):
            ...
            if ((old_val != new_val and (new_val > 0.00000000000000000 or new_val < -0.00000000000000000) and abs(old_val - new_val) > 0.00000000000000000) or
               (axis == 'right_trigger' or axis == 'left_trigger') and new_val == 0 and abs(old_val - new_val) > 0.00000000000000000):
                self.dispatch_event('on_axis', axis, new_val)

    def dispatch_button_events(self, state): ###Button mappings
...
def sample_first_joystick():
    ...
    if arduino is None:
        port = selected_port
        arduino = serial.Serial(port=port, baudrate=460800, timeout=.1)

    # --- Serial output queue and thread ---
    ...
    serial_thread = threading.Thread(target=serial_writer, daemon=True)
    serial_thread.start()
    # --- end serial output queue and thread ---
    ...
    @j.event
    def on_button(button, pressed):
        # Packet: [0x42, button (1B), pressed (1B), 0x0A]
        packet = struct.pack('BBBB', 0x42, button, int(pressed), 0x0A)
        serial_queue.put(packet)
        print('button', button, pressed)

    @j.event
    def on_axis(axis, value):
        # Map axis names to IDs
        axis_map = {
            "left_trigger": 1,
            "right_trigger": 2,
            "l_thumb_x": 3,
        ...
        axis_id = axis_map.get(axis, 0)

        # --- Debounce triggers to reduce spam from l_trigger and right trigger ---
        if not hasattr(on_axis, "last_trigger_values"):
            on_axis.last_trigger_values = {"left_trigger": 0, "right_trigger": 0}
        threshold = 0.001  # Only send if change is greater than this

        if axis in ("left_trigger", "right_trigger"):
            last_val = on_axis.last_trigger_values[axis]
            if abs(value - last_val) < threshold and value != 0:
                return  # Ignore small/noisy changes
            on_axis.last_trigger_values[axis] = value
            value = round(value, 2)  # <-- Multiply trigger value by 10
        # --- End debounce ---

        # Multiply thumb stick values by 10
        # Reduce spam: only send if axis value changes significantly
        if axis in ("l_thumb_x", "l_thumb_y", "r_thumb_x", "r_thumb_y"):
            if not hasattr(on_axis, "last_thumb_values"):
                on_axis.last_thumb_values = {}
            last_val = on_axis.last_thumb_values.get(axis, 0)
            thumb_threshold = 0.004  # Only send if change is greater than this
            if abs(value - last_val) < thumb_threshold and value != 0:
                return  # Ignore small/noisy changes
            on_axis.last_thumb_values[axis] = value
            value = round(value * 2, 3)

        # # --- Round value to 3 decimal places before sending ---
        # value = round(value, 3)
        # # ------------------------------------------------------

        # Packet: [0x41, axis_id (1B), value (float32), 0x0A]
        packet = struct.pack('<BBfB', 0x41, axis_id, float(value), 0x0A)
        serial_queue.put(packet)
    ...
    try:
        while True:
            j.dispatch_events()
            time.sleep(.040)###Lora magic number seems MAX intervals of 50ms
